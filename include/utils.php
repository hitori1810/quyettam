...
function get_user_array($add_blank=true, $status="", $user_id='', $use_real_name=false, $user_name_filter='', $portal_filter=' AND portal_only=0 ', $from_cache = true) {
    ...
    $custom_query = " AND for_portal_only = 0 AND portal_user <> 1";

    if($_REQUEST['searchFormTab'] == 'advanced_search' || $_REQUEST['searchFormTab'] == 'basic_search'){
        $custom_query .= " AND team_set_id IN
        (SELECT
        tst.team_set_id
        FROM
        team_sets_teams tst
        ...
        $status = '';  //Fix bug re-assign User - Listview Search
    }
    //end
    if (empty($locale)) {
        $locale = Localization::getObject();
    ...
    if(empty($user_array)) {
        ...
        if (empty($status)) {
            $query = "SELECT DISTINCT id, first_name, last_name, user_name from users WHERE deleted = 0 ".$portal_filter . $custom_query;
        }
        else {
            $query = "SELECT DISTINCT id, first_name, last_name, user_name from users WHERE status='$status'". $portal_filter. $custom_query;
        }
