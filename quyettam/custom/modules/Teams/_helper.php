<?php
    function getTeamMembers($team_id){
        ...
        CONCAT(IFNULL(l1.first_name, ''),
        ' ',
        IFNULL(l1.last_name, '')) l1_full_name,
        l1.title l1_title,
        ...
        l1.employee_status l1_employee_status
        FROM
        teams
        INNER JOIN
        ...
        WHERE
        (((teams.id = '$team_id')
        AND (l1.employee_status = 'Active')
        AND ((COALESCE(LENGTH(l1.is_admin), 0) > 0))))
        AND teams.deleted = 0";
    ...
    function getRolesForUser($user_id){
        ...
        $i = 0;
        $html = "<select class='selectpicker select_role' data-live-search='true' multiple data-width='200px'>";
    ...
    function getTeamDetail($team_id){
        ...
        $html = "";
        if($team_id != '1')
            $html .= '<input class="button primary" type="button" value="Add User" id="add_user_bt"><br><br>';
        $html .= "<table width='100%' class='table table-striped table-bordered dataTable' id='celebs'>";
        $html .= "<thead><tr>
        <th>".translate('LBL_NAME','Users')."</th>
        <th>".translate('LBL_USER_NAME','Users')."</th>
        <th>".translate('LBL_TITLE','Users')."</th>

        <th>".translate('LBL_DEFAULT_TEAM','Users')."</th>
        <th>".translate('LBL_DEFAULT_SUBPANEL_TITLE','Roles')."</th>
        <th>".translate('LBL_LIST_MEMBERSHIP','Users')."</th>
        <th></th>
        </tr></thead>
        <tbody>";
        $count = 0;
        for($i = 0; $i < count($members); $i++){
            ...
            //$html .= "<td>{$members[$i]['l1_employee_status']}</td>";    
            ...
            if($membership == 'Member')
                ...
                <a class='save_user' style='margin-right: 10px;' href='javascript:void(0)' title = '{$GLOBALS['app_strings']['LBL_EMAIL_SAVE']}'><span class='glyphicon glyphicon-floppy-save' aria-hidden='true'></span></a>
                <a class='remove_user' href='javascript:void(0)' title = '{$GLOBALS['app_strings']['LBL_EMAIL_MENU_REMOVE']}'><span class='glyphicon glyphicon-remove-circle' aria-hidden='true'></a>
                </td>";
            else
                $html .= "<td valign='bottom'><a class='save_user' title = '{$GLOBALS['app_strings']['LBL_EMAIL_SAVE']}' href='javascript:void(0)'><span class='glyphicon glyphicon-floppy-save' aria-hidden='true'></span></a></td>";
    ...
    function getTeamNodes($selected = null){
        $node_arr = array();
        $q1 = "SELECT id, name, parent_id, description FROM teams WHERE private <> 1 AND deleted <> 1";
        $rs1 = $GLOBALS['db']->query($q1);
        while($row = $GLOBALS['db']->fetchByAssoc($rs1)){
            if($row['id'] == '1'){
                $open = true;
