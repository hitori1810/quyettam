...
<tr>
<td width='99%'>
<div width="600" height="450" id="{{$displayParams.key}}_googlemap" style="display:none"><iframe width="700" height="600" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.it/maps?q={$fields.{{$displayParams.key}}_address_street.value|escape:'htmlentitydecode'|escape:'html'|url2html|nl2br}, {$fields.{{$displayParams.key}}_address_city.value|escape:'htmlentitydecode'|escape:'html'|url2html|nl2br}&output=embed"></iframe></div>
...
{$fields.{{$displayParams.key}}_address_country.options[$fields.{{$displayParams.key}}_address_country.value]|escape:'htmlentitydecode'|escape:'html'|url2html|nl2br} <div style="display:inline-block;vertical-align:middle" id="{{$displayParams.key}}_address_country_flag"></div><br>
<input type="button" onclick="{{$displayParams.key}}displayMap(this)" value="View map">
{literal}
<script type="text/javascript"> 
$( document ).ready(function() {
    var flag = $('#{{$displayParams.key}}_address_country').val();
    $('#{{$displayParams.key}}_address_country_flag').html(SUGAR.language.get('app_list_strings','flag_image_dom')[flag]);
});
function {{$displayParams.key}}displayMap($this){
    $('#{{$displayParams.key}}_googlemap').dialog({
        width :600,
        height:450,
    });
}
