...
DCMenu.use('event', 'dd-plugin', 'dd-constrain', 'anim', 'cookie', 'json', 'node-menunav', 'io-base', 'io-form', 'io-upload-iframe', "overlay", function(Y) {
        ...
        while (i < overlays.length) {
            if (!depth || i >= depth) {
                if (i == depth && !overlays[i].visible) {
                    ...
                    if (typeof(overlays[i].bodyNode) != 'undefined' && typeof(overlays[i].bodyNode._node) != 'undefined' && typeof(overlays[i].bodyNode._node.getElementsByTagName('form')[0]) != 'undefined') {
    ...
    function setBody(data, depth, parentid, type, title, extraButton) {
        ...
        if (typeof(data.html) == 'undefined') data = {
            html: data
        };
    ...
    DCMenu.showQuickView2 = function(module, recordID) {
        ...
        overlays[qvDepth].after('render', function(e) {
            this.show();
        });
