	...
	public function getNotifications()
    {
		...
		if( is_admin($current_user) )
        {
		    ...
		    if ( $shouldSkip != '1' )
            {
                ...
                $n = new Notify();
                $unreadNotifications = array_merge($n->getNewNotify(),$n->getEmailCallMeetingTaskNotification(),$n->getAccountContactBirthday());
            ...
            if($unreadNotifications > 0)
            {
                $code = '<div id="notifCount" class="notifCount">'.count($unreadNotifications).'</div>';
                $class = "";
            }

            $image = '<span class="dc_notif_icon" border="0" alt="'.count($unreadNotifications).'" style="background-image: url('.$iconImage.');  "></span>';
		}
        else
        {
