...
$(document).ready(function(){
    ...
    //Generate Dropdown LeadSource
    generate_activity_source();
    lock_lead_source();
    $('#lead_source').on('change', generate_activity_source);
});
...
//Generate Lead Source - Student - Lead - Target

function generate_activity_source(){
    var lead_source         = $('#lead_source').val();
    var activity_selected   = $('#activity').val();
    if(typeof SUGAR.language.languages['app_list_strings'] != 'undefined'){
        var obj_activities      = SUGAR.language.languages['app_list_strings']['json_activity_source_list'][lead_source];
        var activity_name       = SUGAR.language.languages['app_list_strings']['activity_source_list'];
        $('#activity').find('option').remove();
        if(typeof (obj_activities) != 'undefined' && obj_activities != ''){
            var activities= obj_activities.split("|");
            $.each( activities, function( key, value ) {
                $('#activity').append('<option label="'+activity_name[value]+'" value="'+value+'">'+activity_name[value]+'</option>');
            });
            $('#activity').val(activity_selected).show().effect("highlight", {color: '#ff9933'}, 1000);
        }else{
            $('#activity').append('<option label="" value="">-none-</option>');
            $('#activity').val('').hide();
        }
    }
}

function lock_lead_source(){
    if(typeof SUGAR.language.languages['app_list_strings'] != 'undefined'){
        var online_source_list  = Object.keys(SUGAR.language.languages['app_list_strings']['online_source_list']);
        var is_role_mkt         = $('#is_role_mkt').val();
        var lead_source         = $('#lead_source').val();
        if(is_role_mkt == '0'){
            if($.inArray(lead_source, online_source_list) !== -1){
                $("#lead_source option:not(:selected)").prop('disabled', true).addClass('input_readonly');
                $("#activity option:not(:selected)").prop('disabled', true).addClass('input_readonly');
            }else{
                $.each( online_source_list, function( key, value ) {
                    $('#lead_source option[value="' + value + '"]').prop('disabled', true).addClass('input_readonly');
                });
            }
...
//END: Generate Lead Source - Student - Lead - Target