...
<!-- Customize by Thuan Nguyen - OnlineCRM-->
...
<div style="display:inline-block;vertical-align:top" id="{{$country}}_flag"></div>
{literal}
<script type="text/javascript"> 
//Alovipday
$( document ).ready(function() {
    $('#{{$country}}_flag').html(SUGAR.language.get('app_list_strings','flag_image_dom')[$('#{{$country}}').val()]);
              
    $('#{{$country}}').change(function(){
        $('#{{$country}}_flag').html(SUGAR.language.get('app_list_strings','flag_image_dom')[$(this).val()]);
    });
    
    $('#{{$country}}').mousedown(function(event){
        if($('#{{$displayParams.key}}_checkbox').is(':checked')) {
