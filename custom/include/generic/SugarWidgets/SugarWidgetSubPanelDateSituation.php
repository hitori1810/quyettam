...
class SugarWidgetSubPanelDateSituation extends SugarWidgetSubPanelTopSelectButton
{
    ...
    function display($layout_def)
    {
        ...
        if($layout_def['focus']->class_type == "Waiting Class"){
            ...
            $html ='
            <select style="float:left; margin-left: 5px;" id="parent_demo"> <option value = "Leads">Lead</option> <option value = "Contacts">Student</option>
...
//                <div class="vr"></div>';
            ...
            if(!empty($count_inprocess))
                $html .= "<span style='color: #04ADEC; font-size: 20px;'>In progress: $count_inprocess </span> &nbsp;&nbsp;";
            if(!empty($count_notstarted))
                $html .= "<span style='color: #FF8C00; font-size: 20px;'>Not Started: $count_notstarted </span> &nbsp;&nbsp;";
                 if(!empty($count_finish))
                $html .= "<span style='color: #468931; font-size: 20px;'>Finished: $count_finish</span> &nbsp;&nbsp;";
            if(!empty($count_waiting))
                $html .= "<span style='color: #FF8C00; font-size: 20px;'>Waiting: $count_waiting </span> &nbsp;&nbsp;";
            if(!empty($count_outstanding))
                $html .= "<span style='color: #FF0000; font-size: 20px;'>OutStanding: $count_outstanding </span> &nbsp;&nbsp;";
            if(!empty($count_delay))
                $html .= "<span style='color: #FF0000; font-size: 20px;'>Delayed: $count_delay </span> &nbsp;&nbsp;";
            if(!empty($count_demo))
                $html .= "<span style='color: #000000; font-size: 20px;'>Demo: $count_demo </span> &nbsp;&nbsp;";
