...
$(document).ready(function(){
    ...
    $('.remove_pt').live('click',function(){
        ...
        if(confirm("Are you sure to remove current lead from this PT?")){
            ajaxRemovePTFromLead(pt_id);
        }
    });

    $('.remove_demo').live('click',function(){
        var meeting_id = this.getAttribute('id');
        if(confirm("Are you sure to remove current lead from this Demo?")){
            ajaxRemoveDemoFromLead(meeting_id);
        }
    });
...
function handleSelect_lead_pt(){
    open_popup("Meetings",600, 400, "", true, false, {"call_back_function":"set_return_lead_pt","form_name":"DetailView","field_to_name_array":{"id":"id", "meeting_type":"meeting_type"}}, "single", true, 'popupforptdefs');
}
...
function set_return_lead_pt(popup_reply_data, filter){
    ...
    if(name_to_value_array.meeting_type != 'Placement Test'){
        alertify.alert('An error occurred. You have to add to to Schedule Placement Test !!');
        return false;
    }
...
function handleSelect_lead_demo(){
    open_popup("Meetings",600, 400, "", true, false, {"call_back_function":"set_return_lead_demo","form_name":"DetailView","field_to_name_array":{"id":"id","meeting_type":"meeting_type"}}, "single", true, 'popupfordemodefs');
}
...
function set_return_lead_demo(popup_reply_data, filter){
    ...
    if(name_to_value_array.meeting_type != 'Demo'){
        alertify.alert('An error occurred. You have to add to to Schedule Demo !!');
        return false;
    }
    for (var the_key in name_to_value_array) {
        var val = name_to_value_array[the_key].replace(/&amp;/gi, '&').replace(/&lt;/gi, '<').replace(/&gt;/gi, '>').replace(/&#039;/gi, '\'').replace(/&quot;/gi, '"');
