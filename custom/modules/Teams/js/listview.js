...
function toggle_team(){
    ...
    if(is_editing == '1'){
        ...
        $('.team_detail').show();  
        $('#panel_user').show();
        if(count_user > 0 || selectedNode.check_Child_State >= 0)
            $('#delete_bt').hide();
        else
            $('#delete_bt').show();
...
function ajaxGetTeamInfo(teamNode){
    ...
    if(teamNode.id != ''){
        ajaxStatus.showStatus('Loading <img src="custom/images/fb_loading.gif" align="absmiddle">');
        $.ajax({
...
function addRoleTeam(user_id, primary_team_id, roles){
    ajaxStatus.showStatus('Loading <img src="custom/images/fb_loading.gif" align="absmiddle">');
    $.ajax({
        url: "index.php?module=Teams&action=handleUser&sugar_body_only=true",
        type: "POST",
...
$(document).ready(function(){
    ...
    $('.save_user').live('click', function(){
        ...
        var user_id         = $(this).closest('tr').attr('id');
        addRoleTeam(user_id, primary_team_id, roles);   
    });
