...
*/
function getTeamMembers($team_id, $parent_team_id){
    $ext_parent = '';
    if($parent_team_id != '1')
...
//function getTeamsForUsers($array_user){
...
//    WHERE
//    (((users.id IN ("."'".implode("','",$array_user)."'".")) AND l2.private = 0))
//    AND users.deleted = 0
...
function getRolesForUser($user_id, $roles){
...
function getTeamDetail($team_id){
    ...
    if($team_id != '1'){
        $members = getTeamMembers($team->id, $team->parent_id);
        $roles   = getListRole();
        global $mod_strings;
        //Prepare Users List
        $html   = "";
