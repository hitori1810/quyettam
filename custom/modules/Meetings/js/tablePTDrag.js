...
$(document).ready(function () {
    ...
    $(".writing, .speaking, .listening, .reading").live("change", function(){
        ...
        //Sum Total
        debugger;
        var writing     = Numeric.parse($(this).closest('tr').find('input.writing').val());
        var speaking    = Numeric.parse($(this).closest('tr').find('input.speaking').val());
        var listening   = Numeric.parse($(this).closest('tr').find('input.listening').val());
        var reading     = Numeric.parse($(this).closest('tr').find('input.reading').val());
        $(this).closest('tr').find('input.score_result_koc').val(Numeric.toFloat( (writing + speaking + listening + reading ),2,2)).effect("highlight", {color: '#ff9933'}, 1000);
        handleScore($(this).closest('tr').find('input.score_result_koc'),false);
