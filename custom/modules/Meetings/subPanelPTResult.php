    ...
    if($count_pt < 50){//In Case edit
        ...
        if($time_start != $first_time_mt || $duration < $first_duration){
            ...
            while($result = $GLOBALS['db']->fetchByAssoc($result_meeting))
            {
                ...
                if($parent=="Leads"){
                    $html .= getHtmlAddRow($result['lead_first_ec'],$result['primaryid'],$result['j_ptresult_pt_order'],$time_start_rs,$time_end_rs,$result['l1_full_name'],$result['l1_id'],$parent,$result['l1_phone_mobile'],$result['parent_name'],$result_koc_lev,$listening,$speaking,$reading,$writing, $score,$ec_note,$teacher_comment,$attended,false, $birthday);
                }else{
                    $html .= getHtmlAddRow($result['student_first_ec'],$result['primaryid'],$result['j_ptresult_pt_order'],$time_start_rs,$time_end_rs,$result['l2_full_student_name'],$result['l2_id'],$parent,$result['l2_phone_mobile'],$result['parent_name'],$result_koc_lev,$listening,$speaking,$reading,$writing, $score,$ec_note,$teacher_comment,$attended,false , $birthday);
                }
            }
        }
...
function getHtmlAddRow($assigned_to,$result_id,$order,$start,$end,$pt_name,$pt_id,$parent,$pt_phone,$parent_name,$result_koc_lev,$listening,$speaking,$reading,$writing,$score,$ec_note,$teacher_comment,$attended,$showing, $birthdate = ''){
    global $mod_strings;
    ...
    ';
    $tpl_addrow .= '<td align="left" class="priority">'.$order.'</td>';
...
function getSubDemoResult($params){
    ...
    if(count($meetings)<=0){   //In Case Create
        ...
        while($row = $GLOBALS['db']->fetchByAssoc($result_demo)) {
            ...
            if($parent=="Leads"){
                $html .= getDemoAddRow($count_res,$row['l6_full_user_name'],$row['primaryid'],$full_name_lead,$row['l2_id'],$parent,$row['l2_status'],$birthday_lead,$row['l2_gender'],$row['l2_phone_mobile'],$row['parent_name'] ,$row['l3_email_address'],$row['j_ptresult_description'],$row['j_ptresult_attended'],false);
            }else{
                $html .= getDemoAddRow($count_res,$row['l6_full_user_name'],$row['primaryid'],$row['l4_full_student_name'],$row['l4_id'],$parent,$row['l4_contact_status'],$birthday_student,$row['l4_gender'],$row['l4_phone_mobile'], $row['parent_name'], $row['l5_email_address'],$row['j_ptresult_description'],$row['j_ptresult_attended'],false);
            }
        }
    }
...
function getDemoAddRow($count_res, $assigned_to, $result_id,$demo_name,$demo_id,$parent,$status,$birthdate,$gender,$phone_mobile,$parent_name,$email,$ec_note,$attended,$showing){
    if($showing)
        $display = 'style="display:none;"';
