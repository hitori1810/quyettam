        ...
        function display() {
            if(isset($_POST['moduleName']) && !empty($_POST['moduleName']) && isset($_POST['fieldData']) && !empty($_POST['fieldData'])) {
                ...
                if(!in_array('email', $appliedFields)) {
                    if($targetTable == 'C_bookingticket')
                        $statement = ' OR ';
                    else
                        $statement = ' AND ';
                    // Email is not in the select statement
                    $sql = 'SELECT '. join(', ', $selectFields) .' FROM '. $targetTable .' WHERE ('. join($statement, $whereFields) .') AND deleted = 0';
                } else {
                    // Email is in the select statement
                    $sql = 'SELECT '. $targetTable .'.'. join(', ', $selectFields) .' FROM '. $targetTable .', '. $emailJoinTable .', '. $emailTable .
                    ...
                    'AND '. join(' AND ', $whereFields) .' AND '. $targetTable .'.deleted = 0 AND '. $emailJoinTable .'.deleted = 0 AND '. $emailTable .'.deleted = 0';
                }
                ...
                while($record = $db->fetchByAssoc($result)) {
                    ...
                    if(in_array('email', $appliedFields) && $fieldData['id'] == $record['id']) {
                        ...
                        // Has dupplication
                        $name = ($isPerson) ? $record['first_name'] .' '. $record['last_name'] : $record['name'];
                        $assignedUserName = '';
                        ...
                        $row = '<tr>';
                        $row .= '<td><a target="_blank" href="index.php?module='. $moduleName .'&action=DetailView&record='. $record['id'] .'">'. $name .'</a></td>';    
                        $row .= '<td><a target="_blank" href="index.php?module=Users&action=DetailView&record='. $record['assigned_user_id'] .'">'. $assignedUserName .'</a></td>';    
                        $row .= '</tr>';
                    }
                ...
                if($rows != '') {
                    ...
                    $table .= '<table>';
                    $table .= '<thead>';
                    $table .= '<tr><th>'. $mod_strings['LBL_DUPLICATION_NAME_COLUMN'] .'</th><th>'. $mod_strings['LBL_DUPLICATION_ASSIGNED_USER_NAME_COLUMN'] .'</th><tr>';
                    $table .= '</thead>';
