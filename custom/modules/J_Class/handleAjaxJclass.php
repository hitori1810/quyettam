...
switch ($_POST['type']) {
    ...
    case 'ajaxGetStudentList':
        $result = ajaxGetStudentList($_POST['class_id'],$_POST['date_in_content']);
        echo $result;
        break;
    case 'sendSMS':
        $result = sendSMS();
    ...
    if(($_POST['action_demo'] == 'delete')){
        ...
        if($dm_type_student == 'Leads'){
            ...
            if($res_demo)
                return json_encode(array(
                    "success" => "0",
                    "error" => "Lead already demo in the class !",
                ));
            //end
        ...
        if($dm_type_student == 'Contacts'){
            ...
            if($res_demo)
                return json_encode(array(
                    "success" => "0",
                    "error" => "Student already demo in the class !",
                ));
...
function ajaxGetStudentList($classId,$lessonDate){
    global $timedate;
    $lessonDateDb = $timedate->to_db_date($lessonDate,false);
    $student_list = "";
