...
class J_ClassViewEdit extends ViewEdit {
    ...
    public function display(){
        ...
        if(empty($_GET['class_type']) && empty($this->bean->class_type))
            $this->bean->class_type = 'Normal Class';
        ...
        if (!empty($this->bean->id)){
            ...
            //            alert(" EFL tạm ngưng chức năng đổi lịch trong giây lát");
        ...
        while($row = $GLOBALS['db']->fetchByAssoc($rs1)){
            ...
            if($row['team_type'] == 'Adult')
                $htm_koc .= '<option short_course_name="'.$row['short_course_name'].'" koc_id="'.$row['primaryid'].'" '.$str_selected.' json="'.$row['content'].'" label="'.$row['name'].'" value="'.$row['kind_of_course_adult'].'">'.$row['name'].'</option>';
            else
        ...
        if($team_type == 'Adult' && in_array($this->bean->class_type_adult ,$class_type_adult)){
            ...
            $this->ss->assign('dropdown_level','<select name="level[]" style="width: 120px;" id="level" multiple>'.get_select_options($GLOBALS['app_list_strings']['level_program_list'], $this->bean->level).'</select>');
        }else
