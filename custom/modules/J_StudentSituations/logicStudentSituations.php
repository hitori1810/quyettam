...
class logicStudentSituations {
    ...
    function processRecording(&$bean, $event, $arguments){
        if ($_REQUEST['module']=='J_Class' && ($_REQUEST['action']=='DetailView' || $_REQUEST['action']=='SubPanelViewer')){
            ...
            if($team_type == 'Junior'){
                ...
                if(($bean->type == 'Enrolled' || $bean->type == 'Moving In') && $row['class_type'] == 'Normal Class'){
                    if (checkDataLockDate($bean->end_study)){
                        ...
                        $bean->custom_button .= '<input type="button" payment_status="'.$payment_status.'" class="button btn_moving_waiting" value="Move" onClick="window.open(\'index.php?module=J_StudentSituations&action=EditView&return_module=J_StudentSituations&return_action=DetailView&type=Moving%20Out&student_id='.$row['student_id'].'\',\'_blank\')" style="margin-right: 4px;" />';
                    }
                }
                elseif($bean->type == 'OutStanding')
                    $bean->custom_button .= '<input type="button" class="button btn_edit_outstd" student_id="'.$row['student_id'].'" student_name="'.$row['full_student_name'].'" end_study="'.$bean->end_study.'" start_study="'.$bean->start_study.'" situa_id="'.$bean->id.'" value="Edit" style="margin-right: 4px;"/>
                ...
                elseif($bean->type == 'Demo')
                    $bean->custom_button .= '<input type="button" class="button btn_delete_demo" situa_id="'.$bean->id.'" value="Delete" style="margin-right: 4px;"/>';
                elseif($bean->type == 'Delayed'){
                    if($GLOBALS['current_user']->id == '1')
                        $bean->custom_button .= '<input type="button" class="button btn_undo_delay" value="Undo" onclick="undoDelay(\''.$bean->id.'\');" style="margin-right: 4px;"/>';
                    $bean->custom_button .= '';
                } elseif($bean->type == 'Waiting Class' && $row['class_type'] == 'Waiting Class')
                    $bean->custom_button .= '<input type="button" class="button btn_del_waiting" value="Remove" onclick="ajaxDeleteWaitingClass(\''.$bean->id.'\');" style="margin-right: 4px;"/>';
                ...
                if($bean->type == 'Waiting Class' && $row['class_type'] == 'Waiting Class'){
                    ...
                    $bean->custom_button .= '<input type="button"  class="button btn_add_to_class" student_name="'.$row['full_student_name'].'" situa_id="'.$bean->id.'" value="Add To Another Class" style="margin-right: 4px;" onclick="window.open(\'index.php?module=J_Payment&action=DetailView&record='.$row['payment_id'].'\')" />';
                }
            }
            ...
            switch ($bean->status) {
                case "Not Started":
                    $bean->status = "<span style='white-space: nowrap;' class='textbg_orange'>Not Started</span>";
                    break;
                case "In Progress":
                    $bean->status = "<span style='white-space: nowrap;' class='textbg_bluelight'>In Progress</span>";
                    break;
                case "Finished":
                    $bean->status = "<span style='white-space: nowrap;' class='textbg_green'>Finished</span>";
                    break;
            }
            $bean->type = "<span class='$cls'>{$bean->type}</span>";
            if($_REQUEST['action'] != 'Popup'){
                if(empty($bean->no_increasement) || $bean->no_increasement < 0){
                    ...
                    $bean->name = '<a style="white-space: nowrap;" href="index.php?module=J_StudentSituations&action=DetailView&record='.$bean->id.'">'.$bean->name.'</a>';
